...
public class CalculatorService {
	...
	public String postResult(CalculatorModel calculatorModel) {
		...
		if (operationName.equals("Addition")) {
			...
			if (number2 == 0) {
				throw new NotPossibleException("number2 cannot be zero");
			}
			calculate = number1 / number2;
			result = operationName + " of " + number1 + " and " + number2 + " is " + calculate;
			calculatorModel.setResult(calculate);
			...
			throw new NullExceptionHand("Invalid Choice");
		}
		return result;
	...
	public List<String> getResult() {
		...
		for (int i = 0; i < l.size(); i++) {
			String temp = i + 1 + ". " + l.get(i).getOperationName() + " Numbers , number1=" + l.get(i).getNumber1()
					+ " and number2 " + l.get(i).getNumber2();
			result.add(temp);
